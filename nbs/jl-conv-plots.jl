...
# ╔═╡ 5186c4f6-6bd1-11ec-102e-6ff0b8225020
begin
	using Plots
	using PyCall
...
# ╔═╡ 5186c4ce-6bd1-11ec-1c8f-21db9cc8cd65
html"""
...
# ╔═╡ 4b4dd561-c41d-4914-a7ab-f2c47a501c33
@pyimport wandb

# ╔═╡ 6ed49d76-80c9-4442-838e-ccce62c8931d
api = wandb.Api();

# ╔═╡ 5065e324-98dd-4b64-98cf-5b794feac7e4
begin
	volcanic_field_114 = api.run("p0int/ProtoCLR-C/3rhxialw")
	# save the metrics for the run to a csv file
	vf_df = volcanic_field_114.history();
	
	honest_morning = api.run("p0int/ProtoCLR+AE/3d7p5c0o");
	hm = honest_morning.history();
end

# ╔═╡ 1fb2dfcf-6f2b-4a3a-8582-a4c8452dc2d4
moving_average(vs,n) = [sum(@view vs[i:(i+n-1)])/n for i in 1:(length(vs)-(n-1))]

# ╔═╡ 4a64a29f-228a-4751-9682-48cfc39702c7
c = "val_loss"

# ╔═╡ 44e9b912-c9eb-4caa-adc1-c7b6e5dc84c7
begin
	cols1 = vf_df[:columns]
	df = DataFrame(Any[collect(values(vf_df[c])) for c in cols1], map(Symbol, cols1));
	df = filter(Symbol(c) => !isnan, df[:, [Symbol("trainer/global_step"), Symbol(c)]]);
end

# ╔═╡ 700126b0-120c-4c73-a5d7-feba431a7863
begin
	n = 30
	mva = moving_average(df[:, 2], n)
	xs = [[1:length(mva)], mva]
end

# ╔═╡ a0ae0ec2-3d07-4f2e-8188-60fc544bbe7d
begin
	cols2 = hm[:columns]
	df2 = DataFrame(Any[collect(values(hm[c])) for c in cols2], map(Symbol, cols2));
	rename!(df2, :loss => :loss_step, :train_accuracy => :train_accuracy_step)
	df2 = filter(Symbol(c) => !isnan, df2[:, [Symbol("trainer/global_step"), Symbol(c)]]);
end

# ╔═╡ 10425448-d16c-452f-b541-86c606a9a392
begin
	mva2 = moving_average(df2[:, 2], n)
	xs2 = [[1:length(mva2)], mva2]
end

# ╔═╡ 4208e8ea-dc50-4d1f-9fc4-1e00e0590688
begin
	theme(:ggplot2)
	plot(xs[1], xs[2], label="ProtoCLR-C", xlabel="Global Step", ylabel=c, legend=:outerright)
	plot!(xs2[1], xs2[2], label="ProtoCLR", )
	title!("$c vs Steps")
end
