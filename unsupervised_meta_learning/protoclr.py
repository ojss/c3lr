...
__all__ = ['Classifier', 'get_train_images', 'WandbImageCallback', 'TensorBoardImageCallback', 'ProtoCLR']
...
from .proto_utils import (CAE, get_prototypes,
                                                    prototypical_loss, Encoder4L, Decoder4L)
...
class ProtoCLR(pl.LightningModule):
    ...
    lr_decay_step, lr_decay_rate, num_input_channels=1, base_channel_size=64, latent_dim=64,
    ...
    def forward(self, x):
        embeddings = self.encoder(x.view(-1, *x.shape[-3:]))
    ...
    def _get_pixelwise_reconstruction_loss(self, x, x_hat, ways):
        mse_loss = F.mse_loss(x.squeeze(0), x_hat.squeeze(0), reduction='none').sum(dim=[1, 2, 3,]).mean(dim=[0])
        return mse_loss
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.ae:
            mse_loss = self._get_pixelwise_reconstruction_loss(x, x_hat, ways) * self.gamma
